...
async def verify_and_set_cookie(
    file_id: str, token: str, response: Response, action: str = "stream"
):
    """
    ...
    try:
        ...
        if action == "watch":
            ...
            return RedirectResponse(
                url=f"/api/watch/{file_id}", headers=response.headers
            )
        elif action == "download":
            from fastapi.responses import RedirectResponse

            return RedirectResponse(
                url=f"/api/download/{file_id}", headers=response.headers
            )
...
async def watch_file_endpoint(
    ...
    try:
        ...
        if not token_doc:
            raise HTTPException(
                status_code=401, detail="Unauthorized. Expired or invalid cookie."
            )
        ...
        is_owner = (
            requester_id and file_owner_id and int(requester_id) == int(file_owner_id)
        )
        if visibility == "private" and not is_owner:
            raise HTTPException(
                status_code=403, detail="🔒 Private File. Access Denied."
            )
        ...
        max_size_mb = (
            config.get("pro_max_file_size", 4000)
            if plan == "pro"
            else config.get("max_file_size", 2000)
        )
...
async def download_page_endpoint(
    file_id: str, stream_auth_token: str | None = Cookie(None)
):
    """
    ...
    static_dir = (
        Path(__file__).parent.parent / "static" / "pages" / "download_buffer.html"
    )
...
async def stream_file_endpoint(
    ...
    try:
        ...
        if speed_limit > 0:
            return RedirectResponse(
                url=f"/stream/{telegram_file_id}?limit={speed_limit}"
            )
...
async def verify_priority_endpoint(user: int, token: str):
    ...
    try:
        ...
        # Serve success page
        success_page = (
            Path(__file__).parent.parent / "static" / "pages" / "success.html"
        )
...
async def rclone_auth_redirect(user_id: int, client_id: str, client_secret: str):
    ...
    config = await get_config() or {}
    base_url = (
        (config.get("webhook_url") or "").rstrip("/").replace("/webhook/telegram", "")
    )
    if not base_url:
        from web.core.config import settings

        base_url = (
            (settings.WEBHOOK_URL or "").rstrip("/").replace("/webhook/telegram", "")
        )
    ...
    try:
        ...
        try:
            ...
            state_data = json.loads(
                base64.urlsafe_b64decode(padded_state.encode()).decode()
            )
        except Exception:
            raise HTTPException(
                status_code=400,
                detail="Invalid state parameter (CSRF protection blocked request)",
            )
        ...
        if "refresh_token" not in token_data:
            ...
            return {
                "error": "Failed to get refresh token. Did you enable 'Offline Access' and remove any existing access for this app?"
            }
            ...
            if plan == "user":
                keyboard = InlineKeyboardMarkup(
                    [
                        ...
                        [
                            InlineKeyboardButton(
                                "⚙️ My Settings", callback_data="us_settings"
                            )
                        ],
                    ]
                )
                ...
                keyboard = InlineKeyboardMarkup(
                    [
                        [
                            InlineKeyboardButton(
                                "⚙️ Back to Admin Rclone", callback_data="admin_rclone"
                            )
                        ]
                    ]
                )
