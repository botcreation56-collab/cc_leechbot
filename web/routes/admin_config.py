...
async def get_config_endpoint(admin_id: int = Depends(get_current_user)):
    ...
    try:
        ...
        return ConfigResponse(
            ...
            start_message=config.get("start_message", "Welcome to the File Processor Bot!"),
...
# ============================================================
@router.post("/config")
async def update_config_endpoint(request: ConfigRequest, admin_id: int = Depends(get_current_user)):
...
@router.post("/plans")
async def update_plans_endpoint(request: PlansUpdateRequest, admin_id: int = Depends(get_current_user)):
    try:
        from config.settings import get_admin_ids
        if admin_id not in get_admin_ids():
            raise HTTPException(status_code=403, detail="Not authorized")
...
async def trigger_broadcast_endpoint(request: BroadcastRequest, admin_id: int = Depends(get_current_user)):
    try:
        from config.settings import get_admin_ids
        if admin_id not in get_admin_ids():
            raise HTTPException(status_code=403, detail="Not authorized")
        ...
        if not broadcast_id:
            raise HTTPException(status_code=500, detail="Failed to create broadcast draft in DB")
            
        # Spin up the background worker (fire and forget)
        asyncio.create_task(run_broadcast_worker(broadcast_id))
        return {"status": "success", "message": "Global Broadcast initiated in the background!"}
