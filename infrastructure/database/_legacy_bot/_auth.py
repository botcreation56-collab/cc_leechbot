...
async def create_one_time_key(user_id: int, key: str, expires_at: datetime) -> bool:
    """Create one-time key for web auth."""
    try:
        db = get_db()
        # Delete any old keys for this user
        await db.one_time_keys.delete_many({"user_id": user_id})
        ...
        logger.info(f"✅ One-time key created for user: {user_id}")
        return True
